...
type ProgramPlanUpdatedEvent struct {
	SchemaVersion     string `json:"schema_version"`
	EventType         string `json:"event_type"`
	ClientGeneratedID string `json:"client_generated_id"`
	UserID            string `json:"user_id"`
...
func (h *ProgramEventHandlers) HandleProgramPlanUpdated(ctx context.Context, payload []byte) error {
	var event ProgramPlanUpdatedEvent
	if err := json.Unmarshal(payload, &event); err != nil {
		return fmt.Errorf("failed to unmarshal event: %w", err)
	}
	...
	query := `
	UPDATE programs
	SET name = $1, comp_date = $2, training_days_per_week = $3, notes = $4, updated_at = NOW()
	WHERE program_id = $5 AND user_id = $6
	`

	result, err := tx.ExecContext(ctx, query, event.Data.Name, compDate, event.Data.TrainingDaysPerWeek, event.Data.Notes, programID, userID)
	if err != nil {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("program not found or unauthorized: program_id=%s user_id=%s", programID.String(), userID.String())
	}
