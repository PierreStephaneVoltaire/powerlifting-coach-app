...
resource "kubernetes_secret" "app_secrets" {
  ...
  data = {
    ...
    spaces-key              = var.s3_access_key_id
    spaces-secret           = var.s3_secret_access_key
  }
...
resource "kubernetes_secret" "ses_email_secret" {
  ...
  data = {
    smtp-host     = var.ses_smtp_endpoint
    smtp-port     = "587"
...
resource "local_file" "kustomization_patches" {
  ...
  filename = "${var.project_root}/../k8s/overlays/production/kustomization.yaml"
  content  = <<-EOT
...
resource "local_file" "frontend_patch" {
  ...
  filename = "${var.project_root}/../k8s/overlays/production/frontend-patch.yaml"
  content  = <<-EOT
apiVersion: apps/v1
kind: Deployment
metadata:
...
resource "local_file" "auth_service_patch" {
  ...
  filename = "${var.project_root}/../k8s/overlays/production/auth-service-patch.yaml"
  content  = <<-EOT
apiVersion: apps/v1
kind: Deployment
metadata:
...
resource "local_file" "video_service_patch" {
  ...
  filename = "${var.project_root}/../k8s/overlays/production/video-service-patch.yaml"
  content  = <<-EOT
apiVersion: apps/v1
kind: Deployment
metadata:
    ...
    spec:
      ...
      - name: video-service
        ...
        - name: SPACES_ENDPOINT
          value: "https://${var.s3_bucket_domain}"
        - name: SPACES_BUCKET
...
resource "local_file" "media_processor_service_patch" {
  ...
  filename = "${var.project_root}/../k8s/overlays/production/media-processor-service-patch.yaml"
  content  = <<-EOT
apiVersion: apps/v1
kind: Deployment
metadata:
