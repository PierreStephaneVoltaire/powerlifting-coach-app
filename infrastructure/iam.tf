# IAM roles and policies for k3s cluster
...
# Policy for external-dns (Route53 integration)
...
# Policy for EC2 autoscaling and instance management
resource "aws_iam_policy" "ec2_management" {
  name_prefix = "${local.cluster_name}-ec2-mgmt-"
  description = "Policy for EC2 instance management and autoscaling"
  ...
  tags = {
    Name        = "${local.cluster_name}-ec2-mgmt-policy"
    Environment = var.environment
    Project     = var.project_name
  }
}
...
resource "aws_iam_role_policy_attachment" "control_plane_lb_controller" {
  role       = aws_iam_role.control_plane.name
  policy_arn = aws_iam_policy.load_balancer_controller.arn
}

resource "aws_iam_role_policy_attachment" "control_plane_external_dns" {
  role       = aws_iam_role.control_plane.name
  policy_arn = aws_iam_policy.external_dns.arn
}

resource "aws_iam_role_policy_attachment" "control_plane_ec2_management" {
  role       = aws_iam_role.control_plane.name
...
resource "aws_iam_role_policy_attachment" "worker_ec2_management" {
  role       = aws_iam_role.worker.name
  policy_arn = aws_iam_policy.ec2_management.arn
}
...
resource "aws_iam_instance_profile" "control_plane" {
  ...
  tags = {
    Name        = "${local.cluster_name}-control-plane-profile"
    Environment = var.environment
    Project     = var.project_name
  }
